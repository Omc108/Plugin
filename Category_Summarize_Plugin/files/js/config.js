   ...
   // 過去の設定値を代入
    ...
    var checkError = function(table) {
        ...
        $.each(table, function(i, row) {
            ...
            if (row.num_select_formula.indexOf('^') !== -1) error_word.push( i + '行目の集計元フィールドの計算式にべき乗が含まれております');
            if (row.num_select_formula.indexOf('SUM') !== -1) error_word.push( i + '行目の集計元フィールドの計算式にSUMが含まれております');
            // 集計先系
